...
def canonical_reasoning_effort(model: str, effort: Optional[str]) -> Optional[str]:
    if effort is None:
        return None
    if effort == "minimal" and canonical_model_name(model).lower() in {"gpt-5.2"}:
        return "low"
    return effort
            ...
            for c in item.get("content", []) or []:
                ...
                if ctype == "output_text":
                    ...
                    if txt:
                        chunks.append(txt)
                if ctype == "refusal":
                    refusal = c.get("refusal", "model refusal")
        ...
        for c in getattr(item, "content", []) or []:
            ...
            if ctype == "output_text":
                ...
                if txt:
                    chunks.append(txt)
            if ctype == "refusal":
                refusal = getattr(c, "refusal", "model refusal")
                raise RuntimeError(f"Model refusal: {refusal}")
    joined = "\n".join(x for x in chunks if x).strip()
    if not joined:
...
def process_file(
    ...
    cleanup_meta = {
        "method": "openai_hybrid_role_cleanup",
    ...
    return FileReport(
        ...
        api_input_tokens=turn_usage.input_tokens + word_usage.input_tokens,
        api_output_tokens=turn_usage.output_tokens + word_usage.output_tokens,
        api_total_tokens=turn_usage.total_tokens + word_usage.total_tokens,
    )
...
def main() -> None:
    ...
    args.reasoning_effort = canonical_reasoning_effort(args.model, args.reasoning_effort)
